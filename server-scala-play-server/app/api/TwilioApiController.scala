...
class TwilioApiController @Inject()(cc: ControllerComponents, api: TwilioApi) extends AbstractController(cc) {
  /**
    * POST /api/3.18/sms/buyoffer/:appKey?body=[value]&from=[value]&currencyType=[value]
    * @param appKey the application key
    */
  def smsBuyOffer(appKey: String): Action[AnyContent] = Action { request =>
    def executeApi(): TwiMLResponse = {
      ...
      api.smsBuyOffer(appKey, body, from, currencyType)
    }
