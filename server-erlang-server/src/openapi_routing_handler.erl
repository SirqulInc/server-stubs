-module(openapi_routing_handler).
-moduledoc """
...
allowed_methods(Req, #state{operation_id = 'computeRouting'} = State) ->
    {[<<"POST">>], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'computeRouting'} = State) ->
    {[], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'computeRouting'} = State) ->
    {true, Req, State};
...
-spec content_types_provided(cowboy_req:req(), state()) ->
    {[{binary(), atom()}], cowboy_req:req(), state()}.
content_types_provided(Req, #state{operation_id = 'computeRouting'} = State) ->
    {[
...
handle_type_accepted(Req, #state{operation_id = OperationID,
    ...
    {Res, Req1, Context1} = Handler(routing, OperationID, Req, Context),
