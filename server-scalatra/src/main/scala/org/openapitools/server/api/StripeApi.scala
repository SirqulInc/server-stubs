...
class StripeApi(implicit val swagger: Swagger) extends ScalatraServlet
  with FileUploadSupport
  with JacksonJsonSupport
  with SwaggerSupport {
  protected implicit val jsonFormats: Formats = DefaultFormats

  protected val applicationDescription: String = "StripeApi"
  ...
  post("/api/:version/stripe/checkout/session/create", operation(createStripeCheckoutSessionOperation)) {
    ...
    //println("appKey: " + appKey)
            val stripeParameters = params.getAs[String]("stripeParameters")

    //println("stripeParameters: " + stripeParameters)
  }
